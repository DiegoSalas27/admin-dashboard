...
export class BookListComponent implements OnInit {
  ...
  updateSelected(checked:boolean){
    if(checked){
      ...
      this.removeBookList = this.bookList.slice(); // you have to copy a reference not the actual array. It does not copy the reference address
    } else{
      ...
      result => {
        ...
        if(result == "yes"){
          for(let book of this.removeBookList){
             this.removeBookService.sendBook(book.id).subscribe( //ajax call to the back end
            res => {
               // this.getBookList(); //not optimal performance
            },
            error => {
            }
            );
